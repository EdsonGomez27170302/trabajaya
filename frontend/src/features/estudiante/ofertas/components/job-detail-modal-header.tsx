...
export function JobDetailModalHeader({ job }: { job: Job }) {
  return (
    <div className="flex items-center gap-3 pr-10">
      {job.company?.logo_url ? (
        <img src={`${ASSET_BASE_URL}${job.company.logo_url}`} alt={job.company.company_name} className="h-12 w-12 shrink-0 rounded-full object-cover" />
      ) : (
      ...
      {job.is_featured && (
        <span className="ml-auto inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-bold text-amber-700 dark:bg-amber-900/40 dark:text-amber-400">
          <Star className="size-3 fill-amber-500" /> Destacado
        </span>
      )}
    </div>
  );
